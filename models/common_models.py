# Partially borrowed from https://github.com/pliang279/MultiBench/blob/main/unimodals/common_models.py
# Thus follows MIT license.
...
class MLP(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            dims (Iterable): Dimensions of layers.
            ...
            activation: activation function. Defaults to torch.nn.ReLU
        """
        super().__init__()
...
class TwoLayersLSTM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lstm_0 = nn.LSTM(
            indim, hiddim, batch_first=True, bidirectional=isBidirectional
        )
        self.lstm_1 = nn.LSTM(
            ...
            for feat in feats_list:
                if isinstance(feat, nn.Conv2d):
                    pruned_feat = nn.Conv2d(
                        (
                            int(feat.in_channels * prune_factor)
                            if feat.in_channels != 3
                            else 3
                        ),
...
class DAN(torch.nn.Module):
    ...
    def __init__(
        self, indim, hiddim, dropout=False, dropoutp=0.25, nlayers=3, has_padding=False
    ):
    ...
    def forward(self, x):
        ...
        if self.has_padding:
            ...
            mask = torch.arange(embedded.shape[1], device=embedded.device).repeat(
                embedded.shape[0], 1
            ) < x_lens.repeat(-1, 1).repeat(1, embedded.shape[1])
            embedded[~mask] = 0
        # sum pooling
        # pool: B x H
